...
8. **Outbound SMTP (ports 587/465) is blocked at the network level on Railway.** Standard anti-abuse policy on shared PaaS infrastructure - confirmed by direct TCP connection tests from inside the container (both ports time out; port 443 connects fine, including straight to Brevo's own API host). SMTP credentials being correct doesn't matter if the port itself is closed. Fixed by adding a second mail transport, `moodle_phpmailer::brevoSend()`, that sends through Brevo's HTTPS API instead of SMTP - PHPMailer's own `postSend()` already dispatches to a `"{$this->Mailer}Send"` method by convention, so setting `$CFG->brevo_api_key` and `$mailer->Mailer = 'brevo'` in `get_mailer()` (`lib/moodlelib.php`) was enough to wire it in with no other core changes. **Still blocked as of the last test** by a *second*, separate restriction: Brevo's own dashboard IP-allowlist rejects requests from Railway's outbound IP with HTTP 401 `unauthorized`. Fix: log into Brevo, go to **Security > Authorised IPs** (`app.brevo.com/security/authorised_ips`), and either add Railway's current outbound IP or turn the restriction off. Railway's outbound IP can change, so if email starts failing again after working once, re-check this first.
